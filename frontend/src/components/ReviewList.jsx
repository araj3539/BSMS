        ...
        <>
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              ...
              {review.isVerified && (
                <span
                  className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-100"
      ...
      {isReplying && (
        <form onSubmit={handleReplySubmit} className="mt-3 ml-4">
          ...
          <div className="mt-1 flex gap-2">
            <button className="bg-gray-800 text-white px-3 py-1 rounded text-xs">
              Post Reply
            </button>
            <button
              type="button"
...
export default function ReviewList({ reviews, user, bookId, onUpdate }) {
  if (!reviews || reviews.length === 0) {
    return (
      <div className="text-gray-500 mt-4">No reviews yet. Be the first!</div>
    );
  }
