...
export default function ChatWidget() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "system",
      text: "Hi! I'm your literary assistant. Ask me for recommendations!",
    },
  ]);
  ...
  async function sendMessage(e) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "system", text: res.data.reply },
      ]);
    } catch (err) {
      setMessages((prev) => [
        ...prev,
                          ...
                          components={{
                            ...
                            a: ({ node, href, children, ...props }) => (
                              <Link
                                to={href}
                                onClick={() => setIsOpen(false)}
                                className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600 hover:underline decoration-indigo-500"
                                {...props}
                              >
                                {children}
                              </Link>
