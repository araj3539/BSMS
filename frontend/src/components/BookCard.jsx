...
export default function BookCard({ book }) {
  ...
  return (
    <motion.div 
      layout
      className={`group relative bg-white rounded-2xl border border-slate-100 overflow-hidden flex flex-col h-full card-hover ${isOutOfStock ? 'opacity-75 grayscale-[0.5]' : ''}`}
    >
      {/* Image Container */}
      <div className="relative w-full aspect-[2/3] bg-slate-100 overflow-hidden">
        ...
        <button
          ...
          className={`absolute top-3 right-3 p-2.5 rounded-full shadow-md transition-all duration-300 z-10 ${isWishlisted ? 'bg-red-50 text-red-500' : 'bg-white/90 backdrop-blur text-slate-400 hover:text-red-500 hover:scale-110'}`}
          title="Wishlist"
        >
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <Link to={`/book/${book._id}`} className="block mb-1">
            <h3 className="font-serif text-lg font-bold text-slate-900 leading-tight line-clamp-2 group-hover:text-indigo-600 transition-colors" title={book.title}>
            {book.title}
            </h3>
        </Link>
        
        <p className="text-sm text-slate-500 truncate mb-4 font-medium">by {book.author}</p>
        
        <div className="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
            <div className="flex flex-col">
                <span className={`text-xl font-bold ${isOutOfStock ? 'text-slate-400 line-through decoration-red-500/50' : 'text-slate-900'}`}>
                  ₹{book.price}
                </span>
            </div>
            ...
            <div className="flex items-center gap-1 bg-yellow-50 px-2 py-1 rounded-lg border border-yellow-100">
                ...
                <span className="text-xs font-bold text-slate-700">{book.rating || 0}</span>
            </div>
        </div>
      </div>
    </motion.div>
