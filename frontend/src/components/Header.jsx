  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-transform duration-300 ease-in-out border-b border-slate-200/50
        ...
        ${isScrolled ? "bg-white/95 backdrop-blur-md shadow-md pt-1 pb-1" : "bg-white/80 backdrop-blur-sm pt-1 pb-1"}
      `}
    >
            ...
            {user ? (
              <div className="flex items-center gap-4 pl-4 border-l border-slate-200 ml-2">
                ...
                <div className="flex items-center gap-3">
                   {user.role === "customer" && (
                      <>
                        {/* THE FIX: Added My Library to Desktop */}
                        <Link to="/my-library" className={navLinkClass('/my-library')}>Library</Link>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="p-4 space-y-4">
              ...
              <Link to="/cart" onClick={() => setIsMobileMenuOpen(false)} className={navLinkClass('/cart')}>Cart</Link>
              ...
              {user ? (
                <div className="space-y-2 border-t border-slate-100 pt-4 mt-2">
                  ...
                  {user.role === "admin" && (
                    <Link to="/admin" onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-2 text-sm font-bold text-indigo-600 bg-indigo-50 rounded-lg">
                      Admin Dashboard
                    </Link>
                  )}
                  {user.role === "customer" && (
                    <>
                      {/* THE FIX: Added My Library to Mobile */}
                      <Link to="/my-library" onClick={() => setIsMobileMenuOpen(false)} className={navLinkClass('/my-library')}>My Library</Link>
                      <Link to="/my-orders" onClick={() => setIsMobileMenuOpen(false)} className={navLinkClass('/my-orders')}>My Orders</Link>
                      <Link to="/wishlist" onClick={() => setIsMobileMenuOpen(false)} className={navLinkClass('/wishlist')}>Wishlist</Link>
                    </>
                  )}
                  <Link to="/profile" onClick={() => setIsMobileMenuOpen(false)} className={navLinkClass('/profile')}>Profile settings</Link>
                  <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                    Sign Out
                  </button>
                </div>
              ) : (
                <div className="flex flex-col gap-3 pt-2 border-t border-slate-100 mt-2">
                  <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="w-full text-center py-2.5 rounded-xl border border-slate-200 text-slate-700 font-bold text-sm">
                    Log in
                  </Link>
                  <Link to="/signup" onClick={() => setIsMobileMenuOpen(false)} className="w-full text-center py-2.5 rounded-xl bg-slate-900 text-white font-bold text-sm shadow-lg">
                    Sign up
                  </Link>
                </div>
              )}
