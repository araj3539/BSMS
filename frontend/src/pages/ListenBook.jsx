        ...
        if (url.includes("archive.org/compress/")) {
          ...
          if (match && match[1]) {
            ...
            try {
              ...
              let mp3Files = metaData.files.filter(
                (f) => f.name.endsWith(".mp3") && f.source === "original",
              );

              // Fallback just in case there are no 'original' source tags
              if (mp3Files.length === 0) {
                mp3Files = metaData.files.filter(
                  (f) => f.name.endsWith(".mp3") && !f.name.includes("_64kb"),
                );
              }
              ...
              if (mp3Files.length > 0) {
                ...
                const formattedPlaylist = mp3Files.map((file) => ({
                  title:
                    file.title ||
                    file.name.replace(".mp3", "").replace(/_/g, " "),
                ...
                toast.error(
                  "Could not extract playable MP3s from this archive.",
                );
              }
            } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900 flex flex-col font-sans text-white overflow-hidden">
      ...
      <audio
        ...
        autoPlay={isPlaying} /* Natively handle autoplaying the new source */
      ...
      <div className="h-20 flex items-center justify-between px-6 lg:px-12 bg-slate-900 border-b border-slate-800 z-10">
        <h1 className="font-bold text-lg text-slate-200 truncate pr-4">
          {bookData.title}
        </h1>
        <button
          onClick={() => navigate(-1)}
      ...
      <div className="flex-grow flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-indigo-900/40 via-slate-900 to-slate-900 relative">
          <div
            className={`relative w-40 h-40 md:w-56 md:h-56 mb-8 rounded-full bg-gradient-to-tr from-indigo-600 to-purple-600 p-1 shadow-2xl shadow-indigo-900/50 transition-transform duration-700 ${isPlaying ? "scale-105" : "scale-100"}`}
          >
            <div
              className={`w-full h-full rounded-full bg-slate-900 border-4 border-slate-800 flex items-center justify-center ${isPlaying ? "animate-[spin_10s_linear_infinite]" : ""}`}
            >
            ...
            {isPlaying && (
              <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 flex gap-1 items-end h-5">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    className="w-1.5 bg-indigo-500 rounded-t-sm animate-pulse"
                    style={{
          ...
          <div className="w-full max-w-xl bg-slate-800/80 backdrop-blur-md p-6 rounded-3xl border border-slate-700/50 shadow-xl">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-6 mx-auto sm:mx-0">
                ...
                <button
                  onClick={togglePlay}
                  className="w-14 h-14 flex items-center justify-center bg-indigo-600 text-white rounded-full hover:bg-indigo-500 hover:scale-105 transition-all shadow-lg shadow-indigo-600/30"
                >
                  {isPlaying ? (
                    <svg
                      width="20"
        ...
        <div className="w-full lg:w-96 bg-slate-900 border-t lg:border-t-0 lg:border-l border-slate-800 flex flex-col h-64 lg:h-auto">
          ...
          <div className="flex-grow overflow-y-auto custom-scrollbar p-2 space-y-1">
            {playlist.map((track, index) => (
              ...
              >
                <div className="w-6 text-xs opacity-50 font-mono">
                  {index + 1}
                </div>
