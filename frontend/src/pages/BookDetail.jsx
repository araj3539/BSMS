  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <div className="container mx-auto px-4 md:px-6 py-4 md:py-6">
        <Link
          to="/"
          className="text-xs md:text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors flex items-center gap-2"
        >
          &larr; Back to Browse
        </Link>
      </div>
              ...
              <div className="flex flex-col h-full justify-center">
                ...
                <div className="flex items-center gap-4 md:gap-6 mb-6 md:mb-8 border-b border-slate-100 pb-6 md:pb-8">
                  <div className="flex items-center gap-2 bg-yellow-50 px-2 md:px-3 py-1.5 rounded-lg border border-yellow-100">
                    ...
                    <span className="text-xs md:text-sm text-yellow-700 font-bold ml-1 pt-0.5">
                      {book.rating ? book.rating.toFixed(1) : 0}
                    </span>
                  </div>
                  <a
                    href="#reviews"
                    className="text-xs md:text-sm font-medium text-slate-400 hover:text-indigo-600 transition-colors underline decoration-slate-200 hover:decoration-indigo-600 underline-offset-4"
                  >
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mt-auto pt-4 border-t border-slate-50 md:border-0">
                  <div>
                    ...
                    <div
                      className={`text-[10px] md:text-xs mt-1 font-bold uppercase tracking-wide ${book.stock > 0 ? "text-emerald-600" : "text-red-500"}`}
                    >
                      {book.stock > 0
                  ...
                  <div className="flex items-center gap-3 w-full sm:w-auto flex-1 justify-end">
                    <div
                      className={`flex items-center border border-slate-200 rounded-xl bg-slate-50 h-12 ${isOutOfStock ? "opacity-50 pointer-events-none" : ""}`}
                    >
                      <button
                        onClick={() => updateQty(qty - 1)}
                        disabled={qty <= 1}
                        className="w-10 h-full text-slate-500 hover:text-slate-800 hover:bg-white rounded-l-xl transition-colors text-lg"
                      >
                        -
                      </button>
                      ...
                      <button
                        ...
                        className="w-10 h-full text-slate-500 hover:text-slate-800 hover:bg-white rounded-r-xl transition-colors text-lg"
                      >
                        +
                      </button>
                    </div>
                    ...
                    >
                      {adding ? (
                        <div className="loader w-5 h-5 border-white/30 border-t-white"></div>
                      ) : isOutOfStock ? (
                        "Sold Out"
                      ) : (
                        "Add to Cart"
            ...
            <div className="flex items-center justify-between mb-6 md:mb-8">
              ...
              {user && (
                <button
                  ...
                  className="text-xs md:text-sm font-bold text-indigo-600 hover:underline"
                >
                  Write a Review
                </button>
              )}
              ...
              <div id="review-form" className="scroll-mt-24">
                {user ? (
                  ...
                  <div className="bg-slate-100 p-6 md:p-8 rounded-2xl text-center border border-slate-200">
                    ...
                    <Link
                      to="/login"
                      className="inline-block bg-white border border-slate-300 text-slate-700 px-6 py-2 rounded-full font-bold text-sm hover:border-indigo-500 hover:text-indigo-600 transition-all"
                    >
                      Login to Review
                    </Link>
                  </div>
