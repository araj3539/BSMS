...
export default function Cart() {
  ...
  async function remove(i) {
    ...
    if (user) syncCart(user._id, c);
    try {
      await trackInteraction(removedItem.bookId, "REMOVE_CART", {
        source: "cart",
        title: removedItem.title,
        authors: removedItem.authors || [],
        categories: removedItem.categories || [],
      });
    } catch (err) {
      console.error("REMOVE_CART tracking failed", err);
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-6 min-h-[80vh]">
      <h2 className="text-4xl font-serif font-bold text-slate-900 mb-8 tracking-tight">
        Your Cart
      </h2>
      ...
      {loading ? (
        ...
        <div className="flex flex-col lg:flex-row gap-12 animate-pulse">
          <div className="flex-1 space-y-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="flex gap-5 p-4 rounded-2xl border border-slate-200 bg-white"
              >
        ...
        <div className="text-center py-24 bg-white/50 backdrop-blur-sm rounded-3xl border border-dashed border-slate-300 animate-in fade-in zoom-in-95">
          ...
          <Link
            className="px-8 py-3 bg-indigo-600 text-white rounded-full font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all hover:-translate-y-1"
            to="/"
          >
            Start Shopping
          </Link>
                  ...
                  >
                    <div className="w-20 h-28 flex-shrink-0 bg-slate-100 rounded-lg overflow-hidden shadow-sm">
                      {it.coverImageUrl && (
                        <img
                          ...
                          alt={it.title}
                          className={`w-full h-full object-cover ${isOOS ? "grayscale opacity-50" : ""}`}
                        />
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        <h3
                          className={`font-serif text-lg font-bold truncate pr-4 ${isOOS ? "text-slate-400 line-through" : "text-slate-900"}`}
                        >
                          {it.title}
                        </h3>
                        <button
                      ...
                      <div className="flex items-center justify-between">
                        {isOOS ? (
                          <span className="text-xs font-bold text-red-600 bg-red-100 px-3 py-1 rounded-full uppercase tracking-wide">
                            Out of Stock
                          </span>
                        ) : (
                          <div className="flex items-center border border-slate-200 rounded-xl bg-slate-50 h-10 w-fit">
                            <button
                              ...
                              disabled={it.qty <= 1}
                              className="px-3 h-full hover:bg-white rounded-l-xl text-slate-500 disabled:opacity-30 transition-colors font-bold"
                            >
                              -
                            </button>
                            <div className="px-2 w-8 text-center font-bold text-sm text-slate-900">
                              {it.qty}
                            </div>
                            <button
                              onClick={() => update(idx, it.qty + 1)}
                              disabled={it.qty >= it.stock}
                              className="px-3 h-full hover:bg-white rounded-r-xl text-slate-500 disabled:opacity-30 transition-colors font-bold"
                            >
                              +
                            </button>
                          </div>
