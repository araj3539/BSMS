...
export default function OrderDetail() {
  ...
  if (loading) return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-10 animate-pulse">
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-10 pb-24">
      <Link to="/my-orders" className="text-slate-500 hover:text-slate-900 font-medium mb-6 inline-flex items-center gap-2 transition-colors text-sm">
        &larr; Back to Orders
      </Link>
      
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        {/* HEADER */}
        <div className="bg-slate-50 px-6 py-6 border-b border-slate-100 flex flex-col md:flex-row justify-between md:items-center gap-6">
          <div>
            <h1 className="text-xl md:text-2xl font-serif font-bold text-slate-900">Order #{order._id.slice(-8).toUpperCase()}</h1>
            <p className="text-xs md:text-sm text-slate-500 mt-1">Placed on {new Date(order.createdAt).toLocaleDateString()}</p>
          </div>
            ...
            <div className="flex flex-wrap gap-2 w-full md:w-auto">
                {order.status === 'pending' && (
                    <button 
                        ...
                        disabled={cancelling}
                        className="flex-1 md:flex-none text-sm bg-white border border-red-200 text-red-600 px-4 py-2 rounded-lg font-medium hover:bg-red-50 transition-colors disabled:opacity-50 whitespace-nowrap"
                    >
                ...
                <button 
                    ...
                    className="flex-1 md:flex-none text-sm bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg font-medium hover:bg-slate-50 hover:text-indigo-600 transition-colors flex justify-center items-center gap-2 disabled:opacity-50 whitespace-nowrap"
                >
        ...
        <div className="px-6 py-8 bg-white">
            {isCancelled ? (
                <div className="bg-red-50 text-red-700 p-4 rounded-xl text-center font-bold border border-red-100 flex items-center justify-center gap-2 text-sm md:text-base">
                ...
                <>
                    {/* Desktop Tracker */}
                    <div className="hidden md:flex relative justify-between items-center mb-4 max-w-3xl mx-auto">
                    ...
                    {/* Mobile Status Badge (Alternative to Tracker) */}
          ...
          <div className="md:col-span-2 space-y-8">
                ...
                {order.items.map((item, i) => (
                    <div key={i} className="flex justify-between items-center p-4 bg-slate-50/50 rounded-xl border border-slate-100">
                        <div className="flex items-center gap-4">
                            <div className="h-10 w-10 md:h-12 md:w-12 bg-white rounded-lg border border-slate-200 flex items-center justify-center text-lg md:text-xl shadow-sm flex-shrink-0">📖</div>
                            <div className="min-w-0">
                                <p className="font-bold text-slate-800 text-sm md:text-base truncate pr-2">{item.title}</p>
                                <p className="text-xs md:text-sm text-slate-500">Qty: {item.qty} × ₹{formatMoney(item.price)}</p>
                            </div>
                        </div>
                        <span className="font-bold text-slate-900 text-sm md:text-base">₹{formatMoney(item.price * item.qty)}</span>
                    </div>
                ))}
                </div>
            </div>
            ...
            <div>
                ...
                <div className="bg-slate-50 p-6 rounded-xl space-y-3">
                    <div className="flex justify-between text-slate-600 text-sm"><span>Subtotal</span><span>₹{formatMoney(order.subtotal)}</span></div>
                    {order.discount > 0 && (<div className="flex justify-between text-emerald-600 font-medium text-sm"><span>Discount Applied</span><span>- ₹{formatMoney(order.discount)}</span></div>)}
