...
export default function OrderDetail() {
  ...
  async function handleRead(item) {
    trackInteraction(item.bookId, "READ", {
      source: "order_detail",
      title: item.title,
  ...
  async function handleListen(item) {
    trackInteraction(item.bookId, "LISTEN", {
      source: "order_detail",
      title: item.title,
      authors: item.authors,
      categories: item.categories,
  ...
  if (!order)
    return (
      <div className="text-center p-20 text-slate-500">Order not found</div>
    );
  ...
  // Check if they paid and can read digital items
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 md:px-6 py-6 md:py-10 pb-24">
      ...
      <Link
        to="/my-orders"
        className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-indigo-600 transition-colors mb-6"
      >
        <svg
          width="16"
          height="16"
        ...
        <div className="p-5 md:p-8">
          {isCancelled ? (
            <div className="bg-red-50 border border-red-100 rounded-xl p-4 flex items-center gap-3 text-red-700">
              <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center border border-red-100 shadow-sm text-lg">
                ✕
              </div>
              <div>
      ...
      <div className="grid gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          {/* THE FIX: Everything inside this map function has access to the "item" variable! */}
          <div className="divide-y divide-slate-50">
            {order.items.map((item, i) => (
              <div
                key={i}
                className="p-4 flex flex-col md:flex-row gap-4 hover:bg-slate-50/50 transition-colors"
              >
                <div className="flex gap-4 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-slate-900 text-sm md:text-base leading-tight mb-1">
                      {item.title}
                    </p>
                    ...
                    {/* Read / Listen Buttons Safely Inside the Loop */}
                    {canAccessDigitalContent && (
                      <div className="flex flex-wrap gap-3 mt-3">
                        ...
                        <Link
                          to={`/listen/${item.bookId}`}
                          onClick={() => handleListen(item)}
                          className="inline-flex items-center gap-2 px-4 py-2 bg-fuchsia-600 text-white text-xs font-bold rounded-lg hover:bg-fuchsia-700 transition shadow-sm"
                        >
                          <svg
                            width="14"
                            height="14"
        ...
        {order.status === "pending" && (
          <div className="mt-4">
            ...
            <p className="text-xs text-center text-slate-400 mt-2">
              You can only cancel orders while they are pending.
            </p>
          </div>
        )}
      </div>
