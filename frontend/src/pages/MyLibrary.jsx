...
export default function MyLibrary() {
  ...
  const handleRead = async (book) => {
    await trackInteraction(book._id, "READ", {
      source: "my_library",
      title: book.title,
  ...
  const handleListen = async (book) => {
    await trackInteraction(book._id, "LISTEN", {
      source: "my_library",
      title: book.title,
      authors: book.authors,
      categories: book.categories,
      ...
      <div className="mb-8 border-b border-slate-200 pb-6 flex items-end justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">
            You own{" "}
            <span className="text-indigo-600 font-bold">{books.length}</span>{" "}
            titles
          </p>
        </div>
      </div>
      ...
      {books.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
          ...
          <p className="text-slate-500 mt-2 mb-6">
            Discover your next great adventure in our store.
          </p>
          <Link
            to="/"
