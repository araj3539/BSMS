...
export default function AdminDashboard() {
  ...
  }, [nav]); // Removed 'user' dependency to prevent infinite re-fetching loop
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24 px-4 sm:px-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <Link
            to="/admin/books"
            className="flex-1 md:flex-none text-center px-5 py-3 bg-white border border-slate-200 text-slate-700 rounded-xl font-semibold text-sm hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm"
          >
            Manage Inventory
          </Link>
          <Link
            to="/admin/orders"
            className="flex-1 md:flex-none text-center px-5 py-3 bg-indigo-600 text-white rounded-xl font-semibold text-sm shadow-lg shadow-indigo-200 hover:bg-indigo-700 hover:-translate-y-0.5 transition-all"
          >
            View Orders
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <StatCard
          title="Total Revenue"
          value={`₹${stats.cards.totalRevenue.toLocaleString()}`}
          icon="💰"
        ...
        <StatCard
          ...
          alert={stats.lowStockBooks.length > 0}
        />
      </div>
              ...
              {stats.topBooks.map((b, i) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <span
                      className={`w-8 h-8 flex items-center justify-center rounded-lg text-sm font-bold transition-all ${i < 3 ? "bg-indigo-600 text-white shadow-md shadow-indigo-200" : "bg-slate-100 text-slate-500"}`}
                    >
                      {i + 1}
                    </span>
                    <div>
                      <p className="font-bold text-slate-900 text-sm sm:text-base group-hover:text-indigo-600 transition-colors line-clamp-1">
                        {b.title}
                      </p>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-[450px]">
            <div className="p-5 bg-gradient-to-r from-red-50 to-white border-b border-red-100 flex justify-between items-center">
              ...
              <span className="text-xs font-bold bg-white text-red-600 px-2.5 py-1 rounded-md shadow-sm border border-red-100">
                {stats.lowStockBooks.length} items
              </span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-3">
              {stats.lowStockBooks.length === 0 ? (
                ...
                stats.lowStockBooks.map((b) => (
                  <div
                    key={b._id}
                    className="p-4 rounded-2xl bg-white border border-slate-100 shadow-sm hover:border-red-200 hover:shadow-md transition-all group"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <p
                        className="text-sm font-bold text-slate-800 line-clamp-2 leading-tight mr-2"
                        title={b.title}
                      >
                        {b.title}
                      </p>
                    ...
                    <div className="flex justify-between items-center mt-3 pt-3 border-t border-slate-50">
                      ...
                      <Link
                        to={`/admin/books`}
                        className="text-[10px] font-bold text-white bg-slate-900 px-3 py-1.5 rounded-lg hover:bg-indigo-600 transition-colors"
                      >
                        RESTOCK
                      </Link>
                    </div>
...
function StatCard({ title, value, icon, color, alert }) {
  return (
    <div
      className={`relative overflow-hidden bg-white p-6 rounded-3xl shadow-sm border transition-all hover:-translate-y-1 hover:shadow-lg ${alert ? "border-red-200 ring-4 ring-red-50/50" : "border-slate-100"}`}
    >
      <div
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-4">
          <div
            className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${color} flex items-center justify-center text-xl shadow-md shadow-indigo-100 text-white`}
          >
            {icon}
          </div>
        </div>
...
function QuickAction({ to, icon, label }) {
  return (
    <Link
      to={to}
      className="flex flex-col items-center justify-center p-4 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all hover:scale-105 active:scale-95 cursor-pointer backdrop-blur-sm group"
    >
      <span className="text-2xl mb-2 group-hover:-translate-y-1 transition-transform duration-300">
        {icon}
      </span>
