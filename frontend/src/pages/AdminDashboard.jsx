    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">Welcome back, {user.name}. Here's your store overview.</p>
        </div>
        <div className="flex gap-3">
           <Link to="/admin/books" className="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-semibold text-sm hover:bg-slate-50 hover:shadow-sm transition-all">
             Manage Inventory
           </Link>
           <Link to="/admin/orders" className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-semibold text-sm shadow-lg shadow-indigo-200 hover:bg-indigo-700 hover:-translate-y-0.5 transition-all">
             View Orders
           </Link>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
           ...
           <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
              <h3 className="text-lg font-bold text-slate-800 mb-6">Top Performing Books</h3>
           ...
           <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                ...
                <Link to="/admin/reports" className="text-sm text-indigo-600 font-semibold hover:text-indigo-700">See All Reports &rarr;</Link>
              </div>
                ...
                {stats.bestSellers.map((b, i) => (
                  <div key={b._id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                    <div className="flex items-center gap-4">
                      <span className={`w-8 h-8 flex items-center justify-center rounded-lg text-xs font-bold transition-all ${i < 3 ? 'bg-indigo-100 text-indigo-700 ring-2 ring-indigo-50' : 'bg-slate-100 text-slate-500'}`}>
                        {i+1}
                      </span>
                      <div>
                        <p className="font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors">{b.title}</p>
                        <p className="text-xs text-slate-500 font-medium">{b.author}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-slate-900 text-lg">{b.soldCount}</p>
                      <p className="text-[10px] uppercase text-slate-400 font-bold tracking-wider">Sold</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-8">
           ...
           <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-[400px]">
              <div className="p-5 bg-red-50/80 border-b border-red-100 flex justify-between items-center">
                  ...
                  stats.lowStock.map(b => (
                    <div key={b._id} className="p-3 rounded-xl bg-white border border-slate-100 hover:border-red-200 hover:shadow-sm transition-all group">
                      <div className="flex justify-between items-start mb-1">
                        <p className="text-sm font-semibold text-slate-800 line-clamp-1 mr-2" title={b.title}>{b.title}</p>
                        <span className="px-2 py-0.5 bg-red-100 text-red-700 text-[10px] font-bold rounded-md whitespace-nowrap">
                          {b.stock} left
                        </span>
                      </div>
                      <div className="flex justify-between items-center mt-2">
                         <span className="text-xs text-slate-500">Price: ₹{b.price}</span>
...
function StatCard({ title, value, icon, color, alert }) {
  return (
    <div className={`relative overflow-hidden bg-white p-6 rounded-2xl shadow-sm border transition-all hover:-translate-y-1 hover:shadow-lg ${alert ? 'border-red-200 ring-2 ring-red-50' : 'border-slate-100'}`}>
      <div className={`absolute -right-6 -top-6 w-24 h-24 rounded-full bg-gradient-to-br ${color} opacity-10 blur-xl`}></div>
      
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-3">
          <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${color} flex items-center justify-center text-xl shadow-sm text-white`}>
            {icon}
          </div>
