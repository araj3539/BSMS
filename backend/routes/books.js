...
router.get("/", async (req, res) => {
  try {
    ...
    const [total, books] = await Promise.all([
      ...
      Book.find(filter).sort(sortOption).skip(skip).limit(lim),
    ]);
...
router.post("/batch", async (req, res) => {
  try {
    ...
    const books = await Book.find({ _id: { $in: ids } }).select(
      "title price stock coverImageUrl authors categories",
    );
    res.json(books);
  } catch (err) {
    console.error(err);
    ...
    bufferStream
      ...
      .on("end", async () => {
        try {
          ...
          res
            .status(500)
            .json({ msg: "Failed to import books", error: err.message });
        }
      });
  },
);
...
router.get("/:id/read", auth, async (req, res) => {
  try {
    ...
    if (!hasPurchased && !isAdmin) {
      return res
        .status(403)
        .json({ message: "You must purchase this book to read it." });
    }

    // 3. Fetch the book and send the secure URL
    const book = await Book.findById(bookId).select(
      "title ebookUrl audiobookUrl",
    );

    if (!book || (!book.ebookUrl && !book.audiobookUrl)) {
      return res
        .status(404)
        .json({ message: "Digital format not available for this book." });
    }
